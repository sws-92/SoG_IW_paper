...
% Remove NaN From Model data
idx=isnan(model_temp.mtime);
model_temp.mtime(idx)=[];
model_temp.t(:,:,idx)=[];
...
k= waitbar(0,'Creating SSC seasonal cycles...');
for i=1:length(obs.station_lat)

    % Pinpoint closest model_temp points to stations
    Idx=dsearchn([model_temp.lon(:) model_temp.lat(:)],[obs.station_lon(i) obs.station_lat(i)]);
    ...
    waitbar(i/length(obs.station_lat),k);
...
cold_day(idx)=[];obs.station_lon(idx)=[];obs.station_lat(idx)=[];
fitted_harms(:,idx)=[];
...
clearvars -except cold_day fitted_harms amp ampSD cold_daySD ...
    ampCI cold_dayCI obs
save('/ocean/sstevens/IW_project/data/sinfit_SSC_LAYER.mat');
...
save('/ocean/sstevens/IW_project/data/sinfit_SSC_plottting_LAYER.mat');
...
[Xv,Yv]=m_ll2xy(obs.station_lon,obs.station_lat);
[Xgrid,Ygrid]=m_ll2xy(grid_lon,grid_lat);

dday = variogram([Xv' Yv'],cold_day','plotit',false);

[a,c,n,vstruct] = variogramfit(dday.distance,dday.val,[],[],[],'plotit',false);

[zi,zivar] = kriging_use(vstruct,Xv',Yv',cold_day',Xgrid,Ygrid);
...
[CS,CH]=m_contourf(grid_lon,grid_lat,zi_age,[0:10:20 30:15:130],...
    'linecolor','k');
...
s=m_scatter(obs.station_lon,obs.station_lat,5,'x','k');
...
damp = variogram([Xv' Yv'],amp','maxdist',7e4,'plotit',false);

[a,c,n,vstruct] = variogramfit(damp.distance,damp.val,[],[],[],'plotit',false);

[zi,zivar] = kriging_use(vstruct,Xv',Yv',amp',Xgrid,Ygrid);
